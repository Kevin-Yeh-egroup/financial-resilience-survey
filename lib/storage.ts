...
export function saveLastSnapshot(totalScore: number): void {
  ...
  try {
    ...
    const prev = raw ? (JSON.parse(raw) as QuestionnaireSnapshot) : null
    const next: QuestionnaireSnapshot = {
      totalScore,
      previousTotalScore: prev?.totalScore ?? null,
      updatedAt: Date.now(),
    }
    localStorage.setItem(LAST_SNAPSHOT_KEY, JSON.stringify(next))
  } catch (error) {
    console.error("Failed to save last snapshot:", error)
  }
}

export function getLastSnapshot(): QuestionnaireSnapshot | null {
